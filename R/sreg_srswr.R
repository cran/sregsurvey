...
#' @param format character, represents the type of summary of the methodology, 'SIMPLE' or 'COMPLETE'. Default value is 'COMPLETE'.
...
sreg_srswr = function(location_formula,scale_formula,data,fraction,format='COMPLETE',...){
  ...
  if(cond==TRUE){
    ...
    if(format=='SIMPLE'){
      output <- list(sample=sample, estimated_total_y_sreg=t_y_sreg)
      class(output) <- "sregsurvey"
      return(output)
    }
    else{
      output <- list(sampling_design="Simple Random Sampling without Replacement", N =N, n=n, first_order_probabilities=pi,  sample=sample, estimated_total_y_sreg=t_y_sreg)
      class(output) <- "sregsurvey"
      return(output)
    }
}
